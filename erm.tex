...
\item \blue{Espace des hypothèses :} Considérer des fonctions linéaires n'a pas
  ...
  \end{center}
  \item $\fcal = \left\{\xvec \mapsto \sigma\left(\langle \betavec, \xvec \rangle\right) ;  \betavec \in \RR^{p+1}\right\}$
    \end{itemize}
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
  \item \blue{Espace des hypothèses :}
    $\fcal = \{\xvec \mapsto \sigma\left(\langle \betavec, \xvec
  ...
  \item \blue{Minimisation du risque empirique :}
    ...
    \[\argmin_{\betavec \in \RR^{p+1}} \frac1n \sum_{i=1}^n
      -y_i \log \sigma\left(\langle \betavec, \xvec_i \rangle\right)
      - (1-y_i) \log \left(1 - \sigma\left(\langle \betavec, \xvec_i \rangle\right)\right)
    \]
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
  \item \blue{Minimiser}
    {\footnotesize \[J : \betavec \mapsto \sum_{i=1}^n -y_i \log \sigma\left(\langle
      \betavec, \xvec_i \rangle\right) - (1-y_i) \log \left(1 -
      \sigma\left(\langle \betavec, \xvec_i \rangle\right)\right)\]}
  \pause
  \item $J$ est une fonction \blue{convexe} de $\betavec$
  \pause
  \item En utilisant $\sigma^\prime (u) = \sigma(u) (1-\sigma(u)),$
    \[
      \nabla J(\betavec) = \sum_{i=1}^n \left( y_i - \frac1{1 + \exp - \left(
...
\begin{frame}
  ...
  \begin{itemize}
    \item On écrit $P(y_i\,|\,\xvec_i; \betavec)$ la probabilité de prédire la classe $y_i$ (donc de prédire la classe correcte). 
  \item Probabilité que toutes les prédictions sont bonnes est la probabilité jointe de $N$ bonnes prédictions :  
  \begin{equation*}
    \prod_{i=1}^NP(y_i | \xvec_i; \betavec)
  \end{equation*}
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \begin{eqnarray*}
    \betavec^{\ast} &=& \argmax_{\betavec} \prod_{i=1}^NP(y_i | \xvec_i; \betavec) \\
...
\begin{itemize}
  ...
  \item Ces distributions n'étant pas connues, elles doivent être estimées à partir des données d'apprentissage. 
\end{itemize}
