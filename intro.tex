...
\begin{frame}
  ...
  \begin{itemize}
  \item \blue{Planification automatique}
    \begin{itemize}
    \item Trouver une séquence d'actions valides pour atteindre un objectif
    \item Application en robotique, logistique, jeux vidéo
    \end{itemize}
  \item \blue{Systèmes experts}
    \begin{itemize}
    \item utilisent des règles explicites pour encoder l'expertise humaine
    \item Application en médecine, biologie, géologie 
    \end{itemize}
  \item \blue{Constraint Satisfaction Problem (CSP)}
    \begin{itemize}
    \item Trouver des solutions sur plusieurs variables sous contraintes
    \item Application : résoudre de puzzles, affecter des cours ou tâches, coloration de cartes
    \end{itemize}
...
\begin{frame}
  ...
  \item<3-> \red{Algorithme d'apprentissage :}
    \begin{center}
      \begin{tikzpicture}
        \node[text width=5em, align=center] at (0, 5.5) {{Entrées et sorties}};
        \draw[->, draw=MyDarkGrey, thick] (0.9, 5.5) -- (1.9, 5.5);
        \draw [draw=MyOrange, thick] (2, 6) rectangle (4.5, 5) node[midway,
        align=center]{\red{Algorithme} \\ \red{d'apprentissage}};
        \draw[->, draw=MyDarkGrey, thick] (4.6, 5.5) -- (5.6, 5.5);
        \node[text width=5em] at (6.8, 5.5) {\blue{Algorithme classique = modèle}};
        \pause
...
\begin{frame}
  ...
  \item Exemples de \blue{problèmes de classification binaire :}  $\ycal = \{0, 1\}$
    ...
    \item \blue{Contrôle de qualité :} cette pièce est-elle défectueuse ?
    \end{itemize}
  \pause
