...
\begin{frame}
  ...
  \begin{itemize}
    \item On suppose qu'un split $(j,s)$ génère deux régions $R_1$ et $R_2$. 
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item Il s'agit de la proportion d'exemples qui n'apparait pas à la classe majoritaire. 
    \item Ce critère est:
    \begin{itemize}
...
\begin{frame}
  \frametitle{Adaboost}
    \begin{enumerate}
      ...
      \item Apprendre le classifieur $f_m$ à partir de $\dset_m$. Les poids influencent le calul du critère, en remplaçant $p_c(R)$ par :
      \begin{equation}
      \tilde{p}_c(R) = \frac{1}{\sum_iw_i}\sum_{i;y_i=c}w_i
      \end{equation} 
...
\begin{itemize}
  \item AdaBoost est en fait un cas particulier d'une famille de techniques appelees \textit{boosting du gradient (gradient boosting en anglais).}
  \item On considere la fonction de perte exponentielle : 
\begin{eqnarray*}
\{-1, 1\} \times \RR &\rightarrow& \RR \\
y,f(\xvec) \mapsto e^{-yf(\xvec)}
\end{eqnarray*}
\item et la fonction de décision cummulative : 
\begin{equation*}
      F_M(\xvec) = \sum_{m=1}^{M}\alpha_mf_m(\xvec)
\end{equation*}
\item On peut montrer alors que la règle de AdaBoost minimise cette fonction de coût. 
\end{itemize}
\end{frame}

\begin{frame}{Conclusion}
\begin{itemize}
  \item Les forêts aggrègent des résultats de classifieurs faibles
  \item Stratégies : 
  \begin{itemize}
    \item Forêt aléatoire repose sur la diversification et le vote
    \item AdaBoost focalise sur les erreurs (approche collaborative)
  \end{itemize}
  \item AdaBoost est souvent sensible au bruit d'étiquette, mais meilleur pour les jeux de données "propres" 
  \item Les Foréts aléatoires sont souvent plus robustes. 
  \item Ajouter des arbres est habituellement avantageux pour les forêts aléatoires (jusqu'à 500), mais pas nécessairement pour AdaBoost. 
\end{itemize}
