...
\begin{frame}
	\frametitle{Fléau de la dimensionalité}

		
			\begin{figure}[htb]
			%\centering
			\includegraphics[width=.55\textwidth]{CurseDimensionality.png}
			%  \source{adapted from \url{https://cs.stanford.edu/people/karpathy/cnnembed/}}
		\end{figure}
	
	\blue{Conséquence}: Le nombre d'échantillons nécessaire pour couvrir l'espace des données grandit exponentiellement avec la dimension $p$ !
	
\end{frame}


\begin{frame}
	\frametitle{Fléau de la dimensionalité: conséquences}
	
	\begin{itemize}
		\item  Le nombre d'échantillons nécessaire pour couvrir l'espace des données grandit exponentiellement avec la dimension $p$ 
		\item En grande dimension, tous les points sont loin les uns des autres
		\item Les intuitions que l'on a en 2d ou 3d ne sont pas vrais en grande dimension
	\end{itemize}
...
\frametitle{ACP: Méthode 1/3}
 ...
 \end{equation*}
 	\item La matrice de covariance est donc : $\Sigma = \frac1n X^TX$ , $\Sigma \in \mathbb{R}^{p \times p}$
...
\frametitle{ACP: avantages et inconvénients}
  \begin{itemize}
  	...
  	\item Inconvénients : 
  	\begin{itemize}
  		\item maximiser la variance ne veut pas forcément dire maximiser la quantité d'information
...
\begin{frame}
	\frametitle{t-SNE: Motivation}
	Les données ne sont pas toujours séparables linéairement
	\begin{figure}[htb]
	\includegraphics[width=.4\textwidth]{Tsne1.png}
	\hspace{1em}
\visible<2>{	\vrule}
	\hspace{1em}
	\visible<2>{\includegraphics[width=.4\textwidth]{Tsne2.png}}
...
%	\item Many low dimensional representations: PCA, ICA, $\ldots$
	\item Objectif : représentation des données en faible dimension en respectant les relations locales
...
\begin{itemize}
	...
	\item Ils varient selon la densité de voisins, de façon à ce que pour chaque $\xvec_i$ nous ayons à peu près le même nombre de voisins. 
...
\begin{frame}
	\frametitle{t-SNE: Résultat}
Application de t-SNE avec une perplexité de 2 sur l'exemple précédent
	\begin{figure}[htb]
		\includegraphics[width=.4\textwidth]{Tsne1.png}
		\hspace{1em}
\vrule
		\hspace{1em}
\includegraphics[width=.4\textwidth]{Tsne1-solved.png}
...
Application de t-SNE avec une perplexité de 2 sur l'exemple précédent
	\begin{figure}[htb]
		\includegraphics[width=.4\textwidth]{Tsne2.png}
		\hspace{1em}
		\vrule
	\hspace{1em}
	\includegraphics[width=.4\textwidth]{Tsne2-solved.png}		
	\textit{\small{Source:\\ \url{https://distill.pub/2016/misread-tsne/}}}
\end{figure}
\end{frame}
