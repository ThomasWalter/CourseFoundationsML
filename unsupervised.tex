...
% \item Many low-dimensional representations: PCA, ICA, $\ldots$
  ...
  \item We define the probability that $x_j$ is a neighbor of $x_i$, assuming that this probability can be modeled by a Gaussian centered at $x_i$:  
  \begin{equation}
...
\begin{frame}
  \frametitle{1.2 Comparison to Known Labels}
  \begin{itemize}
...
\begin{frame}
  ...
  \item \red{Within-cluster variance}
    ...
    \]
  \item Minimizing the within-cluster variance ⇒ create \blue{homogeneous} clusters
  \end{itemize}
\end{frame}
