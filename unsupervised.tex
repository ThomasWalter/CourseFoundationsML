...
\begin{frame}
\frametitle{Exemple t-SNE : ImageNet}
\begin{itemize}
  \item En vision par ordinateur moderne, les images sont représentées par des vecteurs $\xvec \in \RR^p$, où $p$ est généralement de l'ordre de quelques centaines à quelques milliers.
  \item Ces vecteurs sont ensuite traités pour différentes tâches, par exemple la classification.
  \item Ici, nous allons étudier les représentations vectorielles $\{\xvec_i\}_{i = 1..N}$ d'un sous-ensemble de la base ImageNet, un grand ensemble d'images naturelles.
\end{itemize}
\end{frame}
