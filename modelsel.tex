...
\begin{frame}
  ...
  \item Hypothèses :
    \begin{itemize}
    \item $\dset = (\xvec^1, y^1), \dots, (\xvec^n, y^n)$ \blue{échantillon aléatoire} de $(X, Y)$
    \item $Y = \Phi(X) + \varepsilon$ \hspace{2em} $\mathbb{E}[\varepsilon] = 0$ et $\text{Var}[\varepsilon] = \sigma^2$
    \end{itemize}
  \item \blue{Modèle} $f$ appris $\Rightarrow$ prédiction $f(\xvec)$
    \begin{itemize}
    \item estimation de $\Phi(\xvec)$
    \item réalisation d'une v.a.r. $F_n$ (qui dépend de $\dset$)
  \end{itemize}
  \item \blue{Erreur quadratique moyenne} de l'estimateur :
    \[       \mathbb{E}[(F_n - y)^2] = \mathbb{E}[(F_n - \Phi(\xvec^i) - \varepsilon)^2] = \dots = \text{Biais}(F_n)^2 + \text{Var}(F_n) + \sigma^2 
    \]    
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \item Peut devenir \blue{coûteux en calculs :}
    \begin{itemize}
    \item \textbf{Exemple :} si K=10 pour chaque boucle et 20 valeurs d'hyperparamètre à évaluer 
...
\begin{frame}
  \frametitle{Coefficient de régularisation}
    ...
    \item \red{Coefficient de régularisation} $\lambda > 0$ :
      \begin{itemize}
      \item \blue{hyperparamètre} 
      \item importance relative entre la minimisation du risque empirique et la complexité $\Omega(f)$ du modèle.
  \end{itemize}
    \end{itemize}
